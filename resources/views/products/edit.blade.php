        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden p-4">
            ...
            <div class='grid grid-cols-4 gap-5  '>
                ...
                @forelse ($product->images as $image)
                        ...
                        <div class="flex relative w-full h-48 bg-cover bg-center rounded" style="background-image: url({{ asset_url($image->path) }})">
                            <form action="{{ route('products.images_delete', [$product, $image]) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button href="" class='text-red-500 absolute left-1 bottom-1'>
